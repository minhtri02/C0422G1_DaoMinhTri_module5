...
public interface IBenXeRepository extends JpaRepository<BenXe,Integer> {
    ...
    @Transactional
    @Query(value = "SELECT * FROM ben-xe.ben_xe and bien_so_xe like ?1 ", nativeQuery = true, countQuery = "select count(*) FROM ben-xe.ben_xe and bien_so_xe like ?1")
    Page<BenXe> findAll(Pageable pageable, String key);

    @Modifying
    @Transactional
    @Query(value = "update ben-xe.ben_xe set status=1 where id= :id", nativeQuery = true)
    void delete(@Param("id")Integer id);

    @Transactional
    @Query(value ="select * from xe where id = :id",nativeQuery = true)
    BenXe findByID(@Param("id")Integer id);
